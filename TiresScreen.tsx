...
type TooltipData = {
  ...
  x: number;
  y: number;
  id: string; // Used to distinguish between hover and click states
};

const Tooltip: React.FC<{ data: TooltipData | null }> = ({ data }) => {
  if (!data) return null;
  return (
    <div className="tires-screen__tooltip" style={{ left: data.x, top: data.y }}>
      {data.content}
    </div>
  );
};
...
const TiresScreen: React.FC<TiresScreenProps> = ({ f1LiveTimingState, lapCount, trackStatusInfo, drsStatus }) => {
  ...
  const tiresHelpContent = (
    <>
        <p>
            This screen provides a visual overview of each driver's tire strategy, showing which compounds they have used and for how many laps.
        </p>
        ...
        <ul>
            <li><strong style={{ color: 'var(--red-color)'}}>SOFT:</strong> Red</li>
            <li><strong style={{ color: 'var(--yellow-color)'}}>MEDIUM:</strong> Yellow</li>
            <li><strong style={{ color: 'var(--text-primary)'}}>HARD:</strong> White</li>
            <li><strong style={{ color: 'var(--success-color)'}}>INTERMEDIATE:</strong> Green</li>
            <li><strong style={{ color: 'var(--sidebar-active-link-bg)'}}>WET:</strong> Blue</li>
        </ul>
        ...
        <ul>
            ...
            <li>
                <strong>Stint Details:</strong> Click on any colored bar (a "stint") to see more details, including the exact number of laps completed and the fastest lap time set on that specific tire set.
            </li>
            <li>
         ...
         <hr />
        <p>
            <strong>Note:</strong> During Practice or Qualifying sessions, the timeline is fixed to a shorter lap count for visualization purposes.
        </p>
    </>
  );
  ...
  const renderContent = () => {
    ...
    // Check for absolutely essential data first
    ...
    if (isPracticeOrQualifying) {
        totalLaps = 30; // Use fixed lap count for non-race sessions
    } else {
        // For a race, we require lap count data with a valid total laps.
    ...
    // FIX: Cast Object.values to F1Driver[] to resolve property access error on 'unknown'.
    ...
    return (
      <>
        <div className="tires-screen__chart-container" onClick={() => setTooltipData(null)}>
            {renderTimelineAxis()}
