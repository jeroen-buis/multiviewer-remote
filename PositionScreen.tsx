...
const PositionScreen: React.FC<PositionScreenProps> = ({ f1LiveTimingState, lapCount, trackStatusInfo, drsStatus }) => {
  ...
  const positionHelpContent = (
    <>
        <p>
            This screen provides a visual representation of each driver's race progression, lap by lap. It's a powerful tool for tracking overtakes, undercuts, and how positions have changed from the starting grid.
        </p>
        ...
        <ul>
            <li>
                <strong>Highlighting a Driver:</strong> To focus on a specific driver's performance, simply click on their three-letter abbreviation (TLA) on either the left or right side of the chart. This will highlight their line and dim all others.
            </li>
            <li>
                <strong>Positions Gained/Lost:</strong> At the end of each driver's line, a number in parentheses (e.g., <code>(+1)</code>, <code>(-3)</code>) shows the net number of positions they have gained or lost compared to their starting grid position.
            </li>
            <li>
                <strong>Teammate Lines:</strong> To help distinguish between teammates, who share the same line color, one driver will be represented by a solid line, while their teammate will have a dotted (dashed) line.
            </li>
            <li>
        ...
        <p>
            You may notice that a driver's position sometimes drops a few places on the lap *before* the tire icon appears. This is a normal data artifact related to how pitstops are timed. The position drop is registered when the driver crosses the start/finish line in the pit lane, but the tire change data is associated with the lap number on which the stop occurred. This can create a slight visual offset, but it accurately reflects the official timing data.
        </p>

        <hr />
        <p>
            <strong>Please note:</strong> This chart is only active during race sessions. It will be disabled during Practice and Qualifying as lap-by-lap positions are not relevant in the same way.
        </p>
    </>
  );
  ...
  return (
    <div>
      ...
      />
      <div 
        className="card position-screen__card"
        onClick={(e) => {
